...
- **A real-footage check without its recording is reported SKIPPED, by name — no longer as a
  pass.** Eleven checks re-measure something on a real recording (three real renders, the
  cross-recording compare proof, the ideal-lap table and six measured-figure tables). Each printed a
