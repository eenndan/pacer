...
def test_the_hero_is_no_longer_a_structural_zero_on_the_best_lap():
    ...
    (On the real recordings the same sweep ends at +1.37 s on D24 one chapter and +1.49 s on
    three.)"""
    with _Themed((1440, 900)) as view:
        s = view.session
        best = s.best_lap_id()
...
def test_the_hero_never_prints_a_minus_sign_against_the_ideal():
    ...
    -0.077 s, and on the owner's recordings the floor is -0.255 s (SD_30_08, on its built-in and
    its saved line alike; theme.py tabulates every recording above format_ideal_run). Negative is
