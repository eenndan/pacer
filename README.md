...
- **136 CTest registrations** — Catch2 over the C++ core, plus offscreen Qt suites that build real
  ...
  end-to-end offscreen smoke on every pull request — except the thirteen `footage.*` checks, which
  need a real recording CI does not have, and which CTest therefore lists by name as *Skipped*.
