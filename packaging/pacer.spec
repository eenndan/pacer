...
"""PyInstaller spec: build an UNSIGNED macOS `Pacer.app` for the `studio` desktop app.
...
app = BUNDLE(
    ...
    # The IDENTIFIER keeps its old spelling on purpose: it is not a name anyone reads, and macOS
    # keys per-app state (permissions, saved window state) on it, so renaming it would orphan that.
    bundle_identifier="app.pacer.studio",
    info_plist={
        # == studio.APP_NAME (tests/test_version.py pins it): the menu bar and Finder read these.
        "CFBundleName": "Pacer",
