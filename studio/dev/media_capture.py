...
WINDOW = (1440, 900)
# The Stats page is a scroll column. DATA TRUST sits at its top and the friction circle far below
# it, so no 900 px viewport holds both. The stats shots therefore render into a TALL window:
# nothing about the page is height-dependent, so every crop is the same layout the 900 px window
# shows. It is not tall enough for the whole page any more (it composes into columns at this width
# and grew), so each crop is also scrolled into view first — see `_scroll_to`.
...
def _scroll_to(app: QApplication, stats, first, last) -> None:
    """Scroll the Stats page so the block from `first` down to `last` sits in its viewport — the
    ...
    — stacks UNDER group 0 in the left column: on Sandown 3h the circle sits past the 1500 px
    window's bottom edge. Scrolling puts each crop where a reader would see it."""
    scroll = stats._scroll
    QApplication.sendPostedEvents(None, QEvent.LayoutRequest)
    top = first.mapTo(scroll.widget(), QPoint(0, 0)).y()
    ...
    if bottom > viewport.height():
        raise RuntimeError(f"the block from {type(first).__name__} to {type(last).__name__} is "
                           f"{bottom - first.mapTo(viewport, QPoint()).y()} px tall, more than "
                           f"the {viewport.height()} px Stats viewport")
...
def shot_data_trust(app: QApplication, w: StudioWindow, out_dir: str) -> str:
    ...
    frame; they sit far apart on a scroll column no viewport can show at once, so they are joined
    with `SPACE_XL` of the app's own canvas between them. Nothing is redrawn, moved or retouched.
    The upper half carries its in-app heading; the friction circle has had none since the Stats
    ...
    # crops read as adjacent sections, which they are not — most of the page sits between DATA
...
def shot_clip(w: StudioWindow, out_dir: str, work_dir: str) -> str:
    ...
    poster: a frame of footage is about a tenth the size as a JPEG (105 KB against 1.17 MB)."""
    session = w.session
