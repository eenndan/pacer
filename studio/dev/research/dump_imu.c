// Dump GPS9 (lat,lon,speed2d,speed3d) and GYRO (z,x,y rad/s) and CORI (w,x,y,z quaternion)
// for the first `NPAY` payloads (1 payload = ~1 s) to CSV files, to compare heading sources.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void dump_stream(size_t mp4, uint32_t payloads, uint32_t fourcc, const char *fname, int npay) {
    ...
    for (uint32_t i = g_start; i < payloads && (int)i < g_start + npay; i++) {
        ...
        if (GPMF_OK == GPMF_FindNext(&gs, fourcc, GPMF_RECURSE_LEVELS)) {
            ...
            GPMF_ScaledData(&gs, buf, samples * elems * sizeof(double), 0, samples, GPMF_TYPE_DOUBLE);
            for (uint32_t s = 0; s < samples; s++) {
                double t = in + (out - in) * ((double)s / samples);
                fprintf(f, "%.5f", t);
                for (uint32_t e = 0; e < elems; e++) fprintf(f, ",%.6f", buf[s*elems+e]);
                fprintf(f, "\n");
            }
            free(buf);
        }
...
int main(int argc, char **argv) {
    if (argc < 3) { printf("usage: dump_imu file.mp4 npayloads [start]\n"); return 1; }
    int npay = atoi(argv[2]);
    if (argc >= 4) g_start = atoi(argv[3]);
    size_t mp4 = OpenMP4Source(argv[1], MOV_GPMF_TRAK_TYPE, MOV_GPMF_TRAK_SUBTYPE, 0);
    if (!mp4) { printf("cannot open\n"); return 1; }
