...
| [map_render.py](map_render.py) | The **pure-numpy core of the track map** (extracted from `map_view.py`; every function is numpy-only and paints nothing, though the module is not Qt-free at import — its one `theme` constant, `MAP_RAINBOW_N`, pulls Qt in; pinned in `tests/test_layering.py`'s `QT_REACHING`): value→bucket quantization (`bucketize`), per-bucket draw-array grouping (`bucket_polylines`), grid→points Δ resampling, and the per-channel rainbow computation (speed / Δ / grip). Unit-tested standalone (`test_map_render`). |
