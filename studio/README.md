...
| [overlays.py](overlays.py) | Presentational overlay widgets over StudioWindow: the first-run empty state (**WelcomeView** — drop-zone + Open / Open-demo) and the personal-best share toast (**PBToast**). Self-contained (DI callbacks; no reach into StudioWindow internals). |
...
| [demo.py](demo.py) | **Demo recording resolution** for `--demo` / the welcome "Open demo": resolves a SMALL real lapping clip (fetched once at runtime, never committed). The clips bundled in the `.app` have no real laps, so a first-run user would see an empty studio without this. |
