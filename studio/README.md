...
| [central_view.py](central_view.py) | **CentralView** — the session-scoped central widget for ONE loaded recording: owns the panels (video / map / plots / table / corner-table / consistency / diff-box / chapter banner), the compare + scrub controllers, the shared `PlaybackState`, and the per-frame `tick()`. StudioWindow `setCentralWidget`s a fresh one per load (old one disposed as a unit). |
