...
| [widgets.py](widgets.py) | **Shared Qt widget primitives** (Qt-only, pacer-free, view-agnostic). Today: `WrapLabel`, a word-wrapping `QLabel` that asserts its wrapped height so a layout's *minimum* accounts for it — the fix for a trap that has bitten three surfaces (Help cards, the map's notice plate, the Library privacy note). Use it for any paragraph a layout has to make room for. |
