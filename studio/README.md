...
| Add a keyboard shortcut | `app.py` (the menu / action) + `help_dialog.py` (the shortcut reference) | an offscreen app test |
...
| [help_dialog.py](help_dialog.py) | The **Help-menu dialogs**: the keyboard-shortcut reference (also F1) + the About card. The otherwise-undiscoverable interaction model (Space / M / G / C toggles, ←/→ stepping, chart-cursor scrub) surfaced as two themed `QDialog`s. |
