...
| [stats.py](stats.py) | **Session statistics** (the Stats page) — pure numpy reducers (moving time with a dropout-gap skip, path distance, wall-clock rendering, the pace distribution, peak-g conventions, the half-open lap-window mask, sector medians) + **SessionStats**, the Session-bound DI **service**: whole-recording totals (survive a re-segment), per-lap vmax/avg/peak-g/brake/coast reductions (dropped on re-segment, like the driving channels), session Vmax and the g-g friction-circle cloud. Everything is a REDUCTION of already-validated channels (peak longitudinal g reads the GPS speed-derivative, never the vibration-inflated IMU forward axis); a signal-absent statistic is `None`, never a fake 0. Pacer-free. |
| [stats_panel.py](stats_panel.py) | **StatsView** — the session-statistics dashboard, the 3rd page of the lap panel's stack behind the **Laps \| Corners \| Stats** header toggle (⤢ maximize = a full-window dashboard). Stat-tile groups (SESSION / PACE / SPEED·G / DRIVING), the **g-g friction circle**, per-sector best/median/σ, the **DATA TRUST card** — the IMU↔GPS cross-check's first in-app surface (it was stdout-only) — and a per-lap statistics table (★ best lap, unit-aware speeds). Read-only, refreshed on load/re-segment only (zero per-tick cost); signal-absent sections hide, absent values render an em-dash. |
...
| [prefs.py](prefs.py) | **User preferences** — a tiny JSON store next to the library (same app-support dir, same `_app_support_dir` monkeypatch seam, same atomic write). UI state (last-dir, units, palette); kept separate from the library catalogue. |
