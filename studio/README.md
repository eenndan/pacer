...
1. **Layering.** Only `session.py`, `load.py`, `ingest.py`, `tracks.py` import the `pacer` core —
   every view / controller / helper is **pacer-free** and goes through `Session` (enforced by
...
| [map_render.py](map_render.py) | The **Qt-free pure-numpy core of the track map** (extracted from `map_view.py`): value→bucket quantization (`bucketize`), per-bucket draw-array grouping (`bucket_polylines`), grid→points Δ resampling, and the per-channel rainbow computation (speed / Δ / grip). Unit-tested standalone (`test_map_render`). |
