...
class StatsView(QWidget):
    ...
    def __init__(self, session: Session):
        ...
        # it had been fixed.
        self.ideal = self._mount(col, IdealSection(self.corner_clicked.emit))
    ...
    def _mount(self, col: QVBoxLayout, section):
        ...
        built, exactly as `_make_table` registers the page's own. A TUPLE in that order is a row
        of tiles, laid out on the page's own reflowing grid (`_grid`): tile layout stays here."""
        for widget in section.widgets():
            if isinstance(widget, tuple):
                col.addLayout(self._grid(*widget))
            else:
    ...
    def refresh(self):
        ...
        set_target_tile(self.t_rolling, rolling, ROLLING_TOOLTIP, session)
    ...
    def _set_no_laps_state(self, has_laps: bool):
        ...
        with a lap gate; it has its own composite gate in `IdealSection.refresh`, which runs after
        this and which zero laps already fail (no best lap → no corner basis → no composite). Adding
